...
- The decision model picks an operation and operation-specific target heads in one request: TypeSafe Jev by default, or DeepSeek `deepseek-flash` (thinking disabled, `DEEPSEEK_OUTPUT=prompt|json`) via `DECISION_PROVIDER`. Consume only the selected operation's target.
